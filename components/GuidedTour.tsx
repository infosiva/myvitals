...
export default function GuidedTour({
  ...
  return (
    <>
      <style>{`
        @keyframes tour-in { from { opacity:0; transform:scale(0.95) translateY(6px); } to { opacity:1; transform:scale(1) translateY(0); } }
        @keyframes tour-spot { from { opacity:0; } to { opacity:1; } }
        .tour-tooltip { animation: tour-in 0.25s cubic-bezier(0.4,0,0.2,1) both; }
      ...
      <div
        ...
        style={{
          ...
          boxShadow: `0 8px 32px rgba(0,0,0,0.5), 0 0 0 1px ${accentColor}15`,
        }}
      >
        ...
        <p style={{ fontSize: 15, fontWeight: 800, color: '#fff', marginBottom: 6, letterSpacing: '-0.2px' }}>{step.title}</p>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6, marginBottom: 16 }}>{step.body}</p>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {idx > 0 && (
            <button
              onClick={back}
              style={{ flex: '0 0 auto', padding: '8px 14px', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)' }}
            >
              ←
            </button>
          )}
          <button
            onClick={advance}
            style={{ flex: 1, padding: '9px 14px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', border: 'none', background: `linear-gradient(135deg, ${accentColor}, ${accentColor}cc)`, color: '#000' }}
          >
